...
import { Clock, Save, Loader2, Eye, Trash2 } from "lucide-react";
...
interface AvailabilityWindowPickerProps {
  ...
  isSaving: boolean;
  onSave: (windows: Array<{ day_of_week: number; start_time: string; end_time: string; is_virtual: boolean }>) => Promise<{ success: boolean; error?: string }>;
}

const AvailabilityWindowPicker = ({ windows, isSaving, onSave }: AvailabilityWindowPickerProps) => {
  ...
  const handleSave = async () => {
    ...
    const result = await onSave(windowsToSave);

    if (result.success) {
      toast({
        title: "Availability saved",
        description: `${windowsToSave.length} day(s) configured for appointments.`,
      });
    } else {
      toast({
  ...
  // Calculate preview slots for a day
  const getPreviewSlots = (config: DayConfig) => {
    ...
    for (let hour = startHour; hour < endHour; hour++) {
      const slotStart = new Date(`2000-01-01T${hour.toString().padStart(2, "0")}:00:00`);
          ...
          >
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  This is how clients will see your available appointment times
                </CardDescription>
              </CardHeader>
              <CardContent>
                {enabledDaysCount === 0 ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {DAY_NAMES.map((dayName, dayIndex) => {
                      ...
                      const slots = getPreviewSlots(config);
                ...
                <p className="text-xs text-muted-foreground mt-4 text-center">
                  Each slot is 60 minutes. A 15-minute buffer is applied between appointments.
                </p>
              </CardContent>
            </Card>
          </motion.div>
        ...
        >
          {isSaving ? (
            ...
            <>
              ...
              Save Availability ({enabledDaysCount} day{enabledDaysCount !== 1 ? "s" : ""})
            </>
          )}
        </Button>
      </div>
